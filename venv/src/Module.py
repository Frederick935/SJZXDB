# Python3 模块
"""
在前面几个章节中我们脚本上是用Python解释器来编程，如果你从Python解释器退出再进入，那么你所定义的所有的方法和变量就都消失了
为此Python提供了一个办法，把哪些定义存放在文件中，为一些脚本或者交互式的解释器实例使用，这个文件被称为模块。
模块是一个包含所有你定义的函数和变量的文件，其后缀名为.py，模块可以被别的程序引入，以使用该模块中的函数等功能，这也是
使用Python标准库的方法。
"""
# 下面是使用Python标准库中模块的例子。
import sys
print("命令行参数如下：")
for i in sys.argv:
    print(i)
print('\n\nPython 路径为：', sys.path,'\n')
"""
import sys 引入Python标准库中的sys.py模块，这是引入摸一摸快的方法。
...
# 当解释器遇到import 语句，如果模块在当前的搜索路径就被被导入。
# 搜索路径是一个解释器会先进行搜索的所有目录的列表。如果要导入模块support，需要把命令放在脚本的顶端：
"""
support.py 文件代码
#!/usr/bin/python3
# Filename: support.py
...
党我们使用import语句的时候，Python解释器是怎样找到对应的文件的呢
这就涉及到Python的搜索路径，搜索路径是有一些列目录名组成的，Python解释器就一次从这些目录中去寻找所引入的模块
这看起来很像环境变量，事实上，也可以通过定义环境变量的方式确定搜索路径。
搜索路径是在Python变异或安装的时候确定的，安装新的库应该也会修改。搜索路径被存储在sys模块中的path的变量。在交互式解释器中，输入以下代码：
...
# from...import语句
# Python的from 语句让你从模块中导入一个指定的部分到但该案命名空间中，语法如下：
...
# 深入模块
"""
模块除了方法定义，还可以包括可执行的代码。这些代码一般用来初始化这个模块，这些模块只有在第一次被导入时才会被执行，
每个模块都有各自独立的符号表，在模块内部为所有的函数当作全局符号表来使用。
所有，模块的作者可以放心大胆的在模块内部使用这些全局变量，二不用担心把其他用户的全局变量搞混
从另一个方面，当你确定知道你在做什么的话，你也可以通过modname.item name这样的表示法来方位模块内的函数
...
>>>
"""
# 说明：每个模块都有一个__name__属性，当其值为'__main__'时，表明该该模块自身在运行，否则是被引入。
# __name__与__main__底下是双下划綫，
# dir()函数
...
有些模块直接被构建在解析器里，这些虽然不是一些语言内置的功能，但是他却能高效的使用，甚至是系统级调用也没问题。
这些组件会根据不同的操作系统进行不同形式的配置，比如winreg这个模块就智慧提供给windows系统。
