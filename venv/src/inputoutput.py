# Python3输入和输出
# 在前面几章中，我们其实已经接触了Python的输入和输出功能。本章节我们将具体介绍Python的输入输出
# 输出格式美化
...
#!/usr/bin/python3
# 打开一个文件
f = open("../temp/foo.txt", "w" , encoding='utf-8')
...
f1 = open("../temp/foo.txt","r",encoding='utf-8')
str = f1.read()
print(str)
f1.close()
# f.readline():会从文件中读取单独的一行，换行符为'\n'.f.readline()如果返回一个空字符串，说明已经读取到最后一行。
# f.readlines():将返回该文件中包含的所有行。如果设置为可选参数sizehint，则读取指定长度的字节，并且将这些字节按行分割。
# f.write(string):将string写入到文件中，然后返回写入的字符数。果要写入一些不是字符串的东西, 那么将需要先进行转换
