...
# __iter__()方法返回一个特殊的迭代器对象，这个迭代器对象实现了__next__()方法并通过StopIteration异常标识迭代的王城。
# __next___()方法(Python2里是next()) 会返回下一个迭代器对象
...
# 生成器
# 在Python中，使用了yieId的函数被称为生成器（genertator ）
# 跟普通函数不同的是，生成器是一个返回迭代器的函数，只能用于迭代操作，更简单理解生成器就是一个迭代器
# 在调用生成器运行的过程中，每次遇到yieId时函数会暂停并保存当前所有的运行信息，返回yieId的值，并在下一次执行next()方法时从当前位置继续运行。
